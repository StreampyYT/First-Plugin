			...
			if (args.length == 1) {
				...
				for (int a = 0; a < usersList.size(); a++) {
					...
					if (userRecord.uuid.equals(player.getUniqueId().toString())) {
						userRecord.suffix = args[0];
						player.sendMessage(ChatColor.GOLD + "Je suffix is nu " + ChatColor.translateAlternateColorCodes('&', args[0]) + ChatColor.GOLD + "!");
						return false;
					}
				}
				...
				for (int a = 0; a < usersList.size(); a++) {
					...
					if (userRecord.uuid.equals(target.getUniqueId().toString())) {
						userRecord.suffix = args[1];
						player.sendMessage(ChatColor.GOLD + "Je hebt " + ChatColor.GREEN +  args[0] + ChatColor.GOLD + " suffix verandert naar " + ChatColor.translateAlternateColorCodes('&', args[1]) + ChatColor.GOLD + "!");
						return false;
					}
				}
			}
